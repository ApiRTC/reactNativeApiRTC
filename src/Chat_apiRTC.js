...
export default class Chat_apiRTC extends React.Component {
  ...
  sendMessage() {
    ...
    this.conversation
      ...
      .then(uuid => {
        ...
        this.updateGroupChatContent(
          message,
          this.session.getOrCreateContact(this.session.getId()).getUsername(),
        );
      })
      .catch(err => {
  ...
  updateGroupChatContent(content, sender_id) {
    ...
    this.setState({
      allMessage: this.state.allMessage.set(
        this.nbMessage,
        time + ' => ' + sender_id + ' : ' + content,
      ),
    });
  }
  ...
  render() {
    ...
    function message(ctx) {
      return Array.from(ctx.state.allMessage.values()).map((value, index) => (
